...
$(function() {
  ...
  $('.box').on('propertychange change keyup paste input', function(){
        ...
        $.ajax({
			type:"GET",
			url:"http://localhost:8080/search/index.php",
			data:"String="+tv,
			success:function(res) {
				var out="";
				for (var i in res) {
					out += "<li class=\"content\"><a href=\"/point/?Number=" + res[i].Number + "\">";
					
					out += "<div class=\"w\">";
					out += "<div class=\"label\">" + res[i].Label + "</div>";
					out += "</div>";
					out += "<div class=\"num\">" + res[i].Number + "</div>";
	...
	if (navigator.geolocation) {
		navigator.geolocation.watchPosition(function(pos) {
            ...
            if($('.search').val()=="") $('.box').html("");
			$.ajax({
			type:"GET",
			url:"http://localhost:8080/list/index.php",
			data:"Latitude="+Lat+"&Longitude="+Lon,
			success:function(obj) {
				var out="";
				for (var i in obj) {
					out += "<li class=\"content\"><a href=\"/point/?Number=" + obj[i].Number + "\">";
					...
					out += "</div>";
					out += "</div>";
					out += "<div class=\"num\">" + obj[i].Number + "</div>";
